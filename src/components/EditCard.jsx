...
import { getCardById, updateCard } from "../services/userServices";
...
function EditCard() {
    ...
    const fetchCard = async () => {
        try {
            const response = await getCardById(localStorage.getItem("cardId"));
    ...
    const formik = useFormik({
        ...
        initialValues: {
            ...
            web: defaultCard.web || "",
            image: {
                url: defaultCard.image?.url || "",
                alt: defaultCard.image?.alt || "",
            },
            address: {
                state: defaultCard.address?.state || "",
                country: defaultCard.address?.country || "",
                city: defaultCard.address?.city || "",
                street: defaultCard.address?.street || "",
                houseNumber: defaultCard.address?.houseNumber || "",
                zip: defaultCard.address?.zip || "",
            },
        },
    ...
    useEffect(() => {
        ...
        document.querySelector('.card-img-url-submit').addEventListener("click", (e) => {
        ...
        document.querySelector('.card-img-url-wrapper').addEventListener("click", (e) => {
        ...
        <form className="newcard-form form-form" onSubmit={formik.handleSubmit}>
            <ToastContainer />
                <div className="newcard-left">
                    <h2 className="newcard-title">Edit your card</h2>
                    {inputs.map((input, index) => (
                        index === step && (
                            <div key={input.name}>
                                <h3>Edit Your {input.name.replace('.', '-').replace(/([A-Z])/g, ' $1')}</h3>
                                <input
